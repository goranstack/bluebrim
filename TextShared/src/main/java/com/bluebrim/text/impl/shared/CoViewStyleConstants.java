...
public class CoViewStyleConstants implements CoTextConstants {
	private static AffineTransform m_transform = new AffineTransform(); // for building Font objects
	...
	public static CoEnumValue getBottomRulerAlignment(AttributeSet a) {
		...
		if (align != null)
			...
			return getDefaultRulerAlignment();
	}
	...
	public static CoFont getCoFont(AttributeSet attr, float scale) {
		...
		CoFontAttribute variant = CoFontAttribute.NORMAL_VARIANT; //getVariant( attr );
		CoFontAttribute stretch = CoFontAttribute.NORMAL_STRETCH; //getStretch( attr );
		...
		CoFontFace face =
			CoAbstractFontMapper.getFontMapper().getFontFaceOrFallback(new CoFontFaceSpec(family, weight, style, variant, stretch));

		/*
		if
			( isSmallCaps( attr ) )
		{
			float xh = face.getMetrics().getLineMetrics().getXHeight();
			float ch = face.getMetrics().getLineMetrics().getCapHeight();
			size *= xh / ch;
		}
		*/
	...
	public static String getDefaultFontFamily() {
		//	if(true)return "Arial";
		return (String) CoAbstractFontMapper.getFontMapper().getFallbackFontFamily();
		//	return CoFontManager.getInstance().getFallbackFontFamily();
		//	return "Courier New";
	}
	...
	public static String getFontFamily(AttributeSet a) {
		...
		if (m_dontEvenThinkAboutRenderingType1Fonts) {
			family = "Arial"; // TMP: avoid rendering type 1 fonts
		}
