...
public abstract class CoCompositeView extends View
{
 ...
 */
public CoCompositeView(Element elem)
{
	super(elem);
	children = new View[1];
	nchildren = 0;
 ...
 */
protected abstract void childAllocation(int index, Rectangle2D a);
/**
...
protected Rectangle2D getInsideAllocation(Shape a)
{
	...
	{
		Rectangle2D alloc = a.getBounds2D();
		alloc.setRect( alloc.getX() + left,
			             alloc.getY() + top,
			             alloc.getWidth() - left - right,
			             alloc.getHeight() - top - bottom );
		return alloc;
	}
	return null;
}
	 ...
	 */
	protected abstract View getViewAtPoint(int x, int y, Rectangle2D alloc);
/**
...
protected View getViewAtPosition(int pos, Rectangle2D a)
{
	 ...
	 */
	protected abstract boolean isAfter(int x, int y, Rectangle2D alloc);
	// --- local methods ----------------------------------------------------
	 ...
	 */
	protected abstract boolean isBefore(int x, int y, Rectangle2D alloc);
	/**
	...
	Rectangle2D alloc = getInsideAllocation(a);
	Rectangle2D r0 = alloc.getBounds2D();//new Rectangle(alloc);
	View v0 = getViewAtPosition(p0, r0);
	Rectangle2D r1 = alloc.getBounds2D();//new Rectangle(alloc);
	...
	while (counter < viewCount) {
		...
		if (endView != null) {
		    ...
		    if (endShape instanceof Rectangle2D) {
			retRect.add((Rectangle2D)endShape);
		    }
		    else {
...
/**
 * FIXME:Possible subtile conflict with super class implementation
 * (since JDK 1.3)? /Markus 2002-01-21
 */
...
protected void removeAndReleaseAll()
{
	...
	{
		( (CoReuseableView) children[ i ] ).release();
	}
...
public CoPaintableView getPaintableView(int n)
{
	return (CoPaintableView) getView( n );
}
...
public abstract void paint( CoPaintable g, Shape allocation );
}