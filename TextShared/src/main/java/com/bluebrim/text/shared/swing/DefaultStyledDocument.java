...
/*
 * @(#)src/classes/sov/javax/swing/text/DefaultStyledDocument.java, as122, as122, 20000427 1.5.1.1
 * ===========================================================================
...
/**
 * Copy of javax.swing.text.DefaultStyledDocumentwith a few changes made in order to work around visibility problems
 * when deriving from javax.swing.text.DefaultStyledDocument.
 *
 * Ported this to JDK 1.2.2. /Markus Persson 2000-09-02
 */
public class DefaultStyledDocument extends AbstractDocument implements StyledDocument {
	...
	void insertElement(ElementSpec es) {
		...
		case ElementSpec.JoinNextDirection:
			...
			if((ec.index + 1) < ec.parent.getElementCount())
				...
				// ###3###
			    throw new CoStateInvariantError("Join next to leaf");
			   	// ###3###
	...
	Element join(Element p, Element left, Element right, int rmOffs0, int rmOffs1) {
		...
		return to;
	   	...
	   	// ###3###
		throw new CoStateInvariantError(
	...
	try {
		...
		// ###3###
	    throw new CoStateInvariantError("problem initializing");
	    // ###3###
	} finally {
	    writeUnlock();
	}
...
public Element createLeafElement(Element parent, AttributeSet a, int p0, int p1) {
	...
	return (buffer == null)? null : buffer.getRootElement(); //ibm.2722
	}
	/**
	...
	try {
	    ...
	    // ###2###
	    if (CoSwingTextUtilities.isComposedTextAttributeDefined(attr)) {
...
public boolean isLeftToRight(int p0, int p1) {
	// Dennis 990331: copied from javax.swing.text.AbstractDocument
	// (Note: To change from "package" to public access. /Markus)
