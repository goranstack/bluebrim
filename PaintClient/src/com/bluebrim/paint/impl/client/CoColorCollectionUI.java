...
public class CoColorCollectionUI extends CoDomainUserInterface {
	...
	private void initialize() {
		...
		this.addSpotColorAction = new AbstractAction(CoPaintUIResources.getName(SPOT_COLOR)) {
			public void actionPerformed(ActionEvent e) {
				new addColorTransaction(CoPaintUIResources.getName(SPOT_COLOR), null,
					//getDomain(),
	CoColorCollectionUI.this, getColorCollection().createSpotColor()).execute();
			}
		};
		...
		this.addMultiInkColorAction = new AbstractAction(CoPaintUIResources.getName(MULTI_INK_COLOR)) {
			public void actionPerformed(ActionEvent e) {
				new addColorTransaction(CoPaintUIResources.getName(SPOT_COLOR), null,
					//getDomain(),
	CoColorCollectionUI.this, getColorCollection().createMultiInkColor()).execute();
			}
		};
		...
		this.removeAction = new AbstractAction(CoPaintUIResources.getName(REMOVE)) {
			public void actionPerformed(ActionEvent e) {
				new removeColorTransaction(CoPaintUIResources.getName(REMOVE), null,
					//getDomain(),
	CoColorCollectionUI.this).execute();
			}
		};
		...
		this.addDefaultColorsAction = new AbstractAction(CoPaintUIResources.getName(ADD_RGB_COLORS)) {
			public void actionPerformed(ActionEvent e) {
				new addDefaultColorsTransaction(CoPaintUIResources.getName(ADD_RGB_COLORS), null,
					//getDomain(),
	CoColorCollectionUI.this).execute();
			}
		};
	...
	private boolean areColorsRemoveable(Object[] colors, int[] index) {
		...
		for (int i = 0; i < colors.length; i++) {
			...
			if (!c.canBeDeleted())
				return false;
			if (index[i] < m_startOfMutableColors)
				return false;
		}
		return true;
	}
