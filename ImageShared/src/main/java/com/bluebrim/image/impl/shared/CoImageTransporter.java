...
public class CoImageTransporter implements Serializable, CoImageConstants {
	...
	private final static int DATA_TYPE = DataBuffer.TYPE_INT;
	...
	private final static int[] BIT_MASKS = new int[] {RED_MASK, GREEN_MASK, BLUE_MASK};
	private final static ColorModel COLOR_MODEL = new DirectColorModel(24, RED_MASK, GREEN_MASK, BLUE_MASK);
...
public CoImageTransporter(BufferedImage image) {
	if (image == null)
		throw new IllegalArgumentException("Cannot transport null image. (Silly, I know.)");

	if (image.getType() != IMAGE_TYPE)
		throw new IllegalArgumentException("Cannot transport image other than TYPE_INT_RGB.");

	WritableRaster raster = image.getRaster();

	if (!COLOR_MODEL.isCompatibleRaster(raster))
		throw new IllegalArgumentException("Cannot transport image with incompatible raster.");
