 ...
 */
public CoCMYKColorModel(ColorSpace space, int bits, int cMask,
						int mMask, int yMask, int kMask,
						int transferType) {
	super (space, bits, new int[] {cMask, mMask, yMask, kMask},
		   0, false, Transparency.OPAQUE, transferType);
}
/**
...
/*
	for (int i=0; i < numComponents; i++) {
		components[offset+i] = (pixel & getMask(i)) >>> maskOffsets[i];
	}
*/
	return components;
}
