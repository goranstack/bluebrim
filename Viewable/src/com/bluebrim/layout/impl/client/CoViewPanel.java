...
protected void updateScale( int w, int h )
{
	...
	protected List m_views = new ArrayList(); // [ CoView ]

public CoViewPanel( List views ) // [ CoView ]
{
	this( views, 1 );
}

public CoViewPanel( List views, double scale ) // [ CoView ]
{
	super();
...
private void postSetViews()
{
	...
	invalidate();
	/* revalidate() works much better for me in a JScrollPane
	 * than the previous code which basically did:
	 * getParent().validate();
	 *
	 * Markus 1999-09-13
	 */
...
public void setViews( List views ) // [ CoView ]
{
	m_views.clear();
	m_views.addAll( views );
