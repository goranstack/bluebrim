...
public class CoStrokeRenderer {
	...
	private transient List m_strokeLayerRenderers; // [ StrokeLayerRenderer ]

	// shared rendering stuff
	private static Line2D m_line = new Line2D.Double();

	// "normal" layer renderer
	...
	// layer renderer for symmetric corners
	...
	private static class SymmetricStrokeLayerRendererGP implements StrokeLayerRenderer {
		public List m_shape = new ArrayList(); //Shape m_shape;
		public List m_stroke = new ArrayList(); //Stroke m_stroke;
	...
	private void assureCache() {
		...
		if (strokeWidth == 0) {
			...
			if (symmetry.equals(CoStrokePropertiesIF.NON_SYMMETRIC)) //|| ! ( m_shape instanceof CoRectangleShapeIF ) )
				{
				...
				( symmetry.equals( CoStrokePropertiesIF.SYMMETRIC_BY_SOME_OTHER_MEANS ) )
				{
				assureSymmetricBySomeOtherMeansCache( strokeWidth, stroke );
				*/
			} else {
				// fallback
				assureNonSymmetricCache(strokeWidth, stroke);
	...
	private void assureSymmetricByStretchingCornersCache(float strokeWidth, CoStrokeIF stroke) {
		...
		for (int i = 0; i < I; i++) {
			...
			//		s = s.createExpandedInstance( w / 2 );
			d += D;
		}
	}
...

			{
				...
				if (Double.isInfinite(factor) || Double.isNaN(factor))
					...
					for (int ii = 0; ii < dash.length; ii++)
						newdarray[ii] = (float) dash[ii] * factor;
	...
	private void assureSymmetricByStretchingDashGPCache(float strokeWidth, CoStrokeIF stroke) {
		...
		for (int i = 0; i < I; i++) {
			...
			while (!pi.isDone()) {
				...
				if (phase.size() > 2) {
					//sce.m_cornerStroke.add( ss.getDash().createUndashedStroke( w ));
					// fix corner
					/*GeneralPath gp = new GeneralPath();
					gp.moveTo( x + fLength, y );
					gp.lineTo( x, y );
					gp.lineTo( x, y + sLength );
					*/
				}
			}
	...
	public void paint(CoPaintable g) {
		...
		if (m_strokeLayerRenderers != null) {
			...
			if (!useAntiAliasing() && W < 1 / m_thickestLayerProporionalWidth) {
				// make sure something is painted
