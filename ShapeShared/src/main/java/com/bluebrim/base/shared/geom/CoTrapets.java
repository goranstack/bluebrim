...
/*
 * By Helena Rankegård
 * the following is true for a CoTrapets:
 * m_p1.m_y == m_p2.m_y && m_p3.m_y == m_p4.m_y && m_p1.m_y != m_p3.m_y
 */
...
{
	// assumes that the points in the trapetses are sorted with the first point
	// being the lowest y and x and that the points are stored in clockwise order.
	boolean included = false;
	
	// check if their sides are connected
	if
		( m_p3.equals( trapets.m_p2 ) && m_p4.equals( trapets.m_p1 ) )
	{
		if
			(
				( ( trapets.m_p4.m_x - m_p1.m_x ) / ( trapets.m_p4.m_y - m_p1.m_y ) == ( m_p4.m_x - m_p1.m_x ) / ( m_p4.m_y - m_p1.m_y ) )
			&&
				( ( trapets.m_p3.m_x - m_p2.m_x ) / ( trapets.m_p3.m_y - m_p2.m_y ) == ( m_p3.m_x - m_p2.m_x ) / ( m_p3.m_y - m_p2.m_y ) )
			)
		{
...
{
	return
		m_p1.isEquivalentTo( t.m_p1 )
	&&
		m_p2.isEquivalentTo( t.m_p2 )
	&&
		m_p3.isEquivalentTo( t.m_p3 )
	&&
		m_p4.isEquivalentTo( t.m_p4 );
}
