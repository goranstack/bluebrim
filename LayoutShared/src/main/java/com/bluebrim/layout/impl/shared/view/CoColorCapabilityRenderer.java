...
public void set( boolean hasBlack,
	               boolean hasCyan,
	               boolean hasMagenta,
	               boolean hasYellow,
	               boolean hasFourColor,
	               boolean hasOneOptionalColor,
	               boolean hasTwoOptionalColors )
{
	if
		( hasFourColor )
	{
		...
		if ( hasMagenta ) count++;
		if ( hasYellow ) count++;
		if ( hasOneOptionalColor ) count++;
		else if ( hasTwoOptionalColors ) count += 2;
		...
		if ( hasOneOptionalColor ) m_renderers[ i++ ] = new CoSingleColorCapabilityRenderer();
		else if
			( hasTwoOptionalColors )
		{
...
public void paint( Graphics2D g )
{
	...
	for
		( int i = 0; i < m_renderers.length; i++ )
	{
