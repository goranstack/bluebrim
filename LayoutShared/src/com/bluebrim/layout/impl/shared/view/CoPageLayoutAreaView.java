...
public class CoPageLayoutAreaView extends CoCompositePageItemView {
	...
	private final static long serialVersionUID = -8680222622744236194L;
	...
	boolean validateAddTo(CoCompositePageItemView parent, boolean isDirectParent) {
		return false; // a page layer layout is always root
		/*
		// traverse ancestor path
		while
			( parent != null )
		{
			if
				( parent instanceof CoPageLayerLayoutAreaIF ) // a page layer layout can't be a (grand^n)child of another page layout
			{
				return false;
			}
		
			if
				( parent.m_parent instanceof CoLayoutAreaView )
			{
				parent = (CoLayoutAreaView) parent.getParent();
			} else {
				parent = null;
			}
		}
