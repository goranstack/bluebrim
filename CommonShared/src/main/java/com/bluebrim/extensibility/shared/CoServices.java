...
public class CoServices {
	...
	// JDK1.4 feature, used in ExpandingIterator.readerFor(URL). /Markus
//	private static final Charset UTF_8 = Charset.forName("utf-8");
	...
	public static class ConfigException extends RuntimeException {
		public ConfigException(URL url, int lineNo) {
			super("Illegal configuration-file syntax at line "
				+ lineNo + " in " + url);
		}
		public ConfigException(String className, URL url, int lineNo) {
			super("Illegal provider-class name: " + className
				+ " at line " + lineNo + " in " + url);
		}
	...
	public static Iterator getProviderNames(Class spi) {
		...
		// PENDING: What class loader to use? Use getSystemResources?
		try {
//			return new ExpandingIterator(spi.getClassLoader().getResources(resName));
	...
	public static Object getPreferredProvider(Class spi) {
		...
		while (providers.hasNext()) {
			// PENDING: Add mechanism to somehow specify
			// the preferred provider.
			// Suggestion: Use the Java Preferences, with a node
			// the name of the class (not only package) and MRU-
			// like keys a, b, c and so on for priority. Or keys
			// for each provider and a priority integer ...
			provider = providers.next();
		}
		
		if (provider == null) {
			provider = Proxy.newProxyInstance(spi.getClassLoader(),
				new Class[] {spi, ThrowProxy.class}, new ExceptionThrower(spi));
		}
		return provider;
	}
	...
	public static boolean isReal(Object provider) {
		return !(ThrowProxy.class.isInstance(provider)
			&& Proxy.isProxyClass(provider.getClass()));
	}
