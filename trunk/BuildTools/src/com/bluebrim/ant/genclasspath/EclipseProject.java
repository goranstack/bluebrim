 ...
 * list.</li>
 * <li></li>
 * <strong>classpath </strong> is a path element containing three filesets described below.
 * </ul>
 * 
 ...
 * <code>merged-jars</code> fileset is also used in the jar-target. An example of a merged jar is prepressbase.jar
 ...
 * they should not be merged and they are handled separatly in the deploy target.</li>
 * <li><strong>other-required-jars </strong> contains other jar-files that is part of the compile class path but don't
 * fit in one of the filesets above.</li>
 * </ul>
 ...
 *  
 */
public class EclipseProject implements OrderDependent, Comparable, JarFilePathProvider
{
	...
	private class ExternalJarFile implements Comparable, JarFilePathProvider
	{
		String jarFilePath;
		EclipseProject jarOwner; // The project that has the jar file in the extrenal-jars directory

		ExternalJarFile(String jarFilePath, EclipseProject jarOwner)
		{
	...
	private String name;
	private boolean hasResourceDir = false;

	// Projects checked in the "Projects" tab of the project properties
	private Collection requiredProjects = new ArrayList(); // <EclipseProject>

	// Projects checked in the "Order and export tab" of the project properties
	private Collection exportedRequiredProjects = new ArrayList(); // <EclipseProject>

	// Jars listed in the "Libraries" tab of the project properties
	private Collection externalJars = new ArrayList(); // <ExternalJarFile>

	// Jars checked in the "Order and export tab" of the project properties
	private Collection exportedExternalJars = new ArrayList(); // <ExternalJarFile>
	...
	public EclipseProject(AntClassPathGen projectMap, String name, File classPathFile)
	{
		...
		hasResourceDir = new File(projectDir, "resources").exists();
	}
	...
	public void parseClassPathFile() throws JDOMException, IOException
	{
		...
		while (iterator.hasNext())
		{
			...
			{
				externalJars.add(new ExternalJarFile(path, this));
				if (exported.equals("true"))
					exportedExternalJars.add(new ExternalJarFile(path, this));
			}
		}
	}
	 ...
	 */
	private Collection getExternalJarsInClassPath()
	{
		final Set externalJarsInClassPath = new TreeSet();
	 ...
	 */
	public Collection getProjectsInClasspath()
	{
		final Set projects = new TreeSet();
		Visitor visitor = new Visitor()
		{
	 ...
	 */
	private Collection getRequiredProjects()
	{
		final Set projects = new TreeSet();
		Visitor visitor = new Visitor()
		{
	...
	private Collection getSortedProjectsInClasspath() throws CycleException
	{
		return DependentsSorter.orderDependents(getProjectsInClasspath().iterator());
	}

	private Collection getSortedRequiredProjects() throws CycleException
	{
		return DependentsSorter.orderDependents(getRequiredProjects().iterator());
	}
	...
	public void createAntClassPathFile() throws IOException, CycleException
	{
		...
		// header comment
		doc.addContent(new Comment(" This file have been generated by com.bluebrim.ant.genclasspath.AntClassPathGen. "));
	...
	private void addClassPath(Element classPath)
	{
		...
		// 2) Merged jars
// The merge feature is not wanted at Iprobe
//		Collection projectsToMerge = getProjectsToMerge();
	...
	{
		Collection referedProjects = getProjectsInClasspath();
		int size = referedProjects.size();
		Object[] prerequisites = new Object[size];
		Iterator iter = referedProjects.iterator();
	...
	public int compareTo(Object o)
	{
		return name.compareTo(((EclipseProject)o).name);
	}
	...
	private void injectProjectName(File buildXmlFile) throws IOException
	{
		...
		while ((line = reader.readLine()) != null)
		{
			...
			line = unCommentResourceCopyTask(line);
			writer.write(line);
			writer.newLine();
		}
		reader.close();
	...
	private String unCommentResourceCopyTask(String line)
	{
		if (hasResourceDir)
			return line.replace("<!-- bluebrim.copy.resources / Uncomment if the project has a resource folder-->", "<bluebrim.copy.resources/>");
		else
			return line;
	}
