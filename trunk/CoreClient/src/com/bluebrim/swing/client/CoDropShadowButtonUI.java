 ...
 */
public class CoDropShadowButtonUI extends BasicButtonUI {
/**
 * DropShadowButtonUI constructor comment.
 */
public CoDropShadowButtonUI() {
	super();
}
public Dimension getPreferredSize(JComponent c)
{
	Dimension prefSize 				= super.getPreferredSize(c);
	CoDropShadowButton 	b 			= (CoDropShadowButton)c;
	Point 				dropOffset 	= b.getDropOffset();
	prefSize.width += dropOffset.x;
	prefSize.height += dropOffset.y;
	return prefSize;
}
/**
 */
...
{
		CoDropShadowButton tButton 	= (CoDropShadowButton)jComponent;
		...
		if (tButton.isContentAreaFilled())
		{
			((Graphics2D)g).setPaint(new GradientPaint(
			  new Point(0, 0), tButton.getBackground(),
			  new Point(dimension.width, dimension.height), tButton.getBackground().brighter()));
