...
public class CoContentTool extends CoSelectionTool {
	...
	public CoTool keyPressed(KeyEvent e) {
		if ((e.getModifiers() & MouseEvent.CTRL_MASK) != 0) {
			// ctrl + content tool = selection tool
		...
		if (m_contentView != null) {
			...
			if (c instanceof CoPageItemBoundedContentView) // Ugly, should be dispatched by c
				{
				switch (e.getKeyCode()) {
					case KeyEvent.VK_UP :
	...
	public CoTool mousePressed(MouseEvent e) {
		...
		if (m_contentView != null) {
			...
			if (c instanceof CoPageItemAbstractTextContentView) // Ugly, should be dispatched by c
				{
				...
				if (c instanceof CoPageItemNoContentView) // Ugly, should be dispatched by c
					{
					return this;
				} else
					if (c instanceof CoPageItemBoundedContentView) // Ugly, should be dispatched by c
