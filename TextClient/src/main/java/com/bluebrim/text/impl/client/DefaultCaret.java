...
/*
 * @(#)DefaultCaret.java	1.74 98/12/03
 *
...
import com.bluebrim.text.shared.swing.DefaultStyledDocument;
 ...
 * @version 1.74 12/03/98
 * @see     Caret
 */
 /**
  Dennis 990331: Copied from java.swing.text.
 	               Changed visibility to protected for #setDot och #moveDot.
...
boolean isPositionLTR(int position, Position.Bias bias)
{
	...
	if (doc instanceof DefaultStyledDocument)
	{
		if (bias == Position.Bias.Backward && --position < 0)
			position = 0;
		return ((DefaultStyledDocument) doc).isLeftToRight(position, position);
	}
	return true;
}
	/**
...
public void mouseClicked(MouseEvent e)
{
	if ((e.getModifiers() & InputEvent.BUTTON1_MASK) != 0)
	{
		...
		{
			Action a = (Action) CoActionUtilities.actionsToHashtable( getComponent().getActions() ).get( DefaultEditorKit.selectWordAction );
//			Action a = new DefaultEditorKit.SelectWordAction();
			...
			{
			Action a = (Action) CoActionUtilities.actionsToHashtable( getComponent().getActions() ).get( DefaultEditorKit.selectLineAction );
//				Action a = new DefaultEditorKit.SelectLineAction();
	...
	/**
		Lasse 990206: changed to protected
	*/
	...
	if (dot != this.dot) {
		...
		try {
		    ...
		    throw new CoStateInvariantError("Bad caret position");
		}
	    }
	}
	}
