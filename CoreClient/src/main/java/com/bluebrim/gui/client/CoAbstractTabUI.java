...
public abstract class CoAbstractTabUI extends CoDomainUserInterface implements CoTabUI {
	...
	protected void createValueModels(CoUserInterfaceBuilder builder) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			builder
				.createSubcanvasAdaptor(
					builder
					.addAspectAdaptor(new CoReadOnlyAspectAdaptor(this, iTabData.getKey().toString()) {
	...
	protected void doAfterCreateUserInterface() {
		...
		if (index != -1)
			getTabModelAt(index).valueHasChanged();
		else {
	...
	public int[] getHiddenTabs() {
		...
		for (int i = 0; i < m_tabData.size(); i++) {
			if (m_visibleTabData.contains(m_tabData.get(i)))
				continue;
			else
				hidden[count++] = i;
		}
	...
	protected final CoDomainUserInterface getUserInterfaceAt(int index) {
		if (CoAssertion.TRACE)
			System.out.println("--->getUserInterfaceAt(" + index + ")");
	...
	protected void hideTab(int index) {
		/*
			...
			changed the order to prevent IndexOutOfBoundsException
			Helena Rankegård
		*/
	...
	protected void showTab(CoAbstractUserInterfaceData tabData) {
		// Disables the possibility to add the same tab twice /KG 2001-03-21
	...
	protected CoServerTabData tabData() {
		if (m_serverTabData == null)
			m_serverTabData = getTabData();
