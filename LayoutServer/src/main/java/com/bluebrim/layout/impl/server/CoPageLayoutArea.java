...
public class CoPageLayoutArea extends CoCompositePageItem implements CoPageLayoutAreaIF {
	...
	protected void postShapeChanged() {
		...
		if (m_customGrid != null)
			m_customGrid.setSize(getWidth(), getHeight());
	}
	// subtree was changed, tell page layer

	public void postSubTreeStructureChange(
		boolean added,
		CoShapePageItemIF pageItem,
		CoCompositePageItemIF otherParent) {
		super.postSubTreeStructureChange(added, pageItem, otherParent);

		if (added) {
			boolean doTell = (otherParent == null);
			if (!doTell) {
				CoCompositePageItemIF parent = otherParent; // old parent
				while (parent != this) {
					if (parent == null) {
						doTell = true;
						break;
			...
			boolean doTell = (otherParent == null);
			if (!doTell) {
				CoCompositePageItemIF parent = otherParent; // new parent
				while (parent != this) {
					if (parent == null) {
						doTell = true;
						break;
	...
	public void xmlVisit(CoXmlVisitorIF visitor) {
		...
		if (m_size != null)
			visitor.exportAsGOIorObject(XML_PAGE_SIZE, m_size);
	...
	protected void collectState(CoPageItemIF.State s, CoPageItemIF.ViewState viewState) {
		...
		S.m_isLeftSideOfSpread = false; // = m_isSpread && (m_page != null) && isLeftSide();
		S.m_isRightSideOfSpread = false; // = m_isSpread && (m_page != null) && !isLeftSide();
	...
	public void setSpread(boolean s) {
		...
		if (columnGrid != null)
			columnGrid.setSpread(s);

		doAfterPageSizeChanged();
	}

	/**
	 *	The object in m_page is not a subModel so why is this done? /Göran S 
	 */
	public void xmlAddSubModel(String name, Object subModel, CoXmlContext context) {
	...
	public static CoXmlImportEnabledIF xmlCreateModel(Object superModel, Node node, CoXmlContext context) {
		...
		if (superModel instanceof CoPage)
			area = new CoPageLayoutArea((CoPage) superModel);
		else
			area = new CoPageLayoutArea(null);
	...
	public void xmlInit(NamedNodeMap map, CoXmlContext context) {
		...
		if (context.useGOI()) {
			...
			if (goiStr != null) {
				CoPageSizeIF s =
					((CoPageSizeRegistry) context.getValue(CoPageSizeRegistry.class)).lookupPageSize(new CoGOI(goiStr));
